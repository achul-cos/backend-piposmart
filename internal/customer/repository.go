...
func ownerWhere(actor Actor, params ListParams) (string, []any) {
	...
	if params.SubscriptionStatus != "" {
		...
		switch st {
		case "SUBSCRIBE", "BERLANGGANAN", "ACTIVE":
		...
		case "NOT_SUBSCRIBE", "TIDAK BERLANGGANAN", "NOT SUBSCRIBE", "TIDAK_BERLANGGANAN":
			where = append(where, `NOT EXISTS (
				SELECT 1 FROM subscriptions s
				JOIN outlets ot4 ON ot4.id = s.outlet_id
				WHERE ot4.owner_id = o.id AND s.deleted_at IS NULL
		...
		default:
			where = append(where, "(o.status = ? OR o.status LIKE ?)")
			args = append(args, st, like(st))
		}
	}
...
func globalOutletWhere(actor Actor, params ListParams) (string, []any) {
	...
	if params.OwnerKeyword != "" {
		regexpPattern := wordBoundaryRegexp(params.OwnerKeyword)
		likePattern := like(params.OwnerKeyword)
		where = append(where, "((o.code REGEXP ? OR o.name REGEXP ?) OR (o.code LIKE ? OR o.name LIKE ? OR o.brand_name LIKE ?))")
		args = append(args, regexpPattern, regexpPattern, likePattern, likePattern, likePattern)
	}
	if params.Code != "" {
		where = append(where, "ot.code LIKE ?")
		args = append(args, like(params.Code))
